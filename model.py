...
from semantic import cider_compute
...
class Loss(nn.Module):
    ...
    def DAA(self, scores, sems, temp = 0.001):
        N = len(scores)

        score_broad = scores.unsqueeze(-1)
        ...
        return self.daa_weight * (1 - ASP)

    def SmoothAP(self, scores, weight=25, temp = 0.001):
        N = len(scores)  # batch size
        ...
        score_rank = diag_mask * self.sigmoid(tensor = score_diff, temp = temp)
        score_rank_all = score_rank.sum(0) + 1
